...
public class RocketLauncherItem extends Item {
  ...
  public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
    ...
    boolean hasAmmo =
        player.isCreative()
            || player.getInventory().countItem(ModItems.ROCKET.get()) > 0;
  ...
  public void releaseUsing(ItemStack stack, Level level, LivingEntity entityLiving, int timeLeft) {
    ...
    if (hasAmmo) {
      if (!level.isClientSide) {
        RocketLauncherProjectileEntity bullet = new RocketLauncherProjectileEntity(player, level);
        bullet.shootFromRotation(player, player.getXRot(), player.getYRot(), 2.0F, 2.5F, 0F);
        bullet.setBaseDamage(4);
        ...
        if (!player.isCreative()) {
          removeAmmo(player, ModItems.ROCKET.get());
        }
      }
