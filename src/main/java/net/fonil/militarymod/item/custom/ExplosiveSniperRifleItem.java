...
public class ExplosiveSniperRifleItem extends Item {
  public ExplosiveSniperRifleItem(Properties properties) {
    super(properties);
  }
  ...
  public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
    ...
    boolean hasAmmo =
        player.isCreative()
            || player.getInventory().countItem(ModItems.EXPLOSIVE_SNIPER_RIFLE_AMMO.get()) > 0;
  ...
  public void releaseUsing(ItemStack stack, Level level, LivingEntity entityLiving, int timeLeft) {
    ...
    if (hasAmmo) {
      if (!level.isClientSide) {
        ExplosiveSniperRifleProjectileEntity bullet = new ExplosiveSniperRifleProjectileEntity(player, level);
        ...
        if (!player.isCreative()) {
          removeAmmo(player, ModItems.EXPLOSIVE_SNIPER_RIFLE_AMMO.get());
        }
      }
